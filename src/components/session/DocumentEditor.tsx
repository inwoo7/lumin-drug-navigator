  ...
  const handleExportPDF = async () => {
    ...
    try {
      ...
      for (let i = 1; i <= totalPages; i++) {
        ...
        pdf.text('Lumin Placeholder Logo', margin, margin - 15);
        ...
        pdf.text(docTitle, pdfWidth - margin - titleWidth, margin - 15);
        pdf.setDrawColor(200);
        pdf.line(margin, margin - 5, pdfWidth - margin, margin - 5);

        pdf.text('Generated by: Placeholder User', margin, pdfHeight - margin + 15);
        const pageStr = `Page ${i} of ${totalPages}`;
        const pageStrWidth = pdf.getStringUnitWidth(pageStr) * pdf.getFontSize() / pdf.internal.scaleFactor;
        pdf.text(pageStr, pdfWidth - margin - pageStrWidth, pdfHeight - margin + 15);
